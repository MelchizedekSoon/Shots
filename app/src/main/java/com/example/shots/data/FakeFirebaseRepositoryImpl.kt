...
class FakeFirebaseRepositoryImpl @Inject constructor(
    ...
    override fun logOut() {
        Log.d(
            "FirebaseRepository",
            "loggedInUser before = ${firebaseAuth.currentUser ?: ""}"
        )
        Log.d(
            "FirebaseRepository",
            "getYourUserId() before = ${firebaseAuth.currentUser?.displayName ?: ""}"
        )
        firebaseAuth.signOut()
        Log.d(
            "FirebaseRepository",
            "loggedInUser after = ${firebaseAuth.currentUser ?: ""}"
        )
        Log.d(
            "FirebaseRepository",
            "getYourUserId() after = ${firebaseAuth.currentUser?.displayName ?: ""}"
        )
    }
    ...
    override suspend fun getMetadataFromStorage(mediaIdentifier: String): String {
...
//                    }
    ...
    override suspend fun fetchMediaUrls(userId: String): MutableMap<String, String> {
...
//    }
    ...
    override suspend fun writeBookmarksToFirebase(
        userId: String,
    ...
    override suspend fun getBookmarksFromFirebase(userId: String): List<String> {
        if (userId.isBlank()) {
            return emptyList()
        }
...
//                        val sentLikeValue = document.getString(field)?.removePrefix("sentLike-")
    ...
    override suspend fun deleteSentLikeFromFirebase(userId: String?): Boolean {
        if (userId == null) {
            Log.d("FirebaseRepository", "Invalid userId: null")
            return false // Error
        }
    ...
    override suspend fun getReceivedLikesFromFirebase(userId: String): List<String> {
        if (userId.isBlank()) {
            return emptyList()
        }
...
//                        val receivedLikeValue = document.getString(field)
    ...
    override suspend fun deleteReceivedLikeFromFirebase(userId: String?): Boolean {
        if (userId == null) {
            Log.d("FirebaseRepository", "Invalid userId: null")
            return false // Error
        }
    ...
    override suspend fun deleteSentShotFromFirebase(userId: String): Boolean {
        val storageRef = firebaseStorage.reference
    ...
    override suspend fun getSentShotsFromFirebase(userId: String): List<String> {
        if (userId.isBlank()) {
            return emptyList()
        }
...
//                }
    ...
    override suspend fun getReceivedShotsFromFirebase(userId: String): List<String> {
...
//                        val receivedShotValue = value
    ...
    override suspend fun deleteReceivedShotFromFirebase(userId: String): Boolean {
        val storageRef = firebaseStorage.reference
    ...
    override suspend fun writeBlockedUserToFirebase(
        ...
        return try {
            ...
            userWhoBlockedYouData["userWhoBlockedYou-${getYourUserId()}"] = getYourUserId()
    ...
    override suspend fun getBlockedUsersFromFirebase(userId: String): List<String> {
        if (userId.isBlank()) {
            return emptyList()
        }
...
//                        val blockedUserValue = document.getString(field)?.removePrefix("blockedUser-")
    ...
    override suspend fun getUsersWhoBlockedYouFromFirebase(userId: String): List<String> {
        if (userId.isBlank()) {
            return emptyList()
        }
...
//            val userDataWithoutMedia = userData.filterKeys { !it.startsWith("media") }
    ...
    override fun getTypeFromUri(context: Context, uri: Uri): String? {
        val contentResolver = context.contentResolver
        return contentResolver.getType(uri)
    }

    override suspend fun uploadMediaItemsToStorage(
        userId: String,
        mediaItems: MutableMap<String, Uri>,
        userData: MutableMap<String, Any>,
        context: Context
    ...
    override suspend fun uploadImageToStorage(imageUri: Uri, mediaIdentifier: String): Boolean {

        return try {
            if (getYourUserId() != null) {
                deleteMediaFromFirebase(getYourUserId(), mediaIdentifier)
            }
            val storageRef = firebaseStorage.reference
            val userRef = storageRef.child("users/${getYourUserId()}")
    ...
    override fun getImageFromStorage(imageUri: Uri, mediaIdentifier: String): Boolean {
        return try {
            val storageRef = firebaseStorage.reference
            val userRef = storageRef.child("users/${getYourUserId()}")
            val imageRef = userRef.child("gallery")
    ...
    override fun uploadVideoToStorage(videoUri: Uri, mediaIdentifier: String): Boolean {
        return try {
            val storageRef = firebaseStorage.reference
            val userRef = storageRef.child("users/${getYourUserId()}")
    ...
    override fun uploadProfileVideoToStorage(videoUri: Uri): Boolean {
        return try {
            val storageRef = firebaseStorage.reference
            val userRef = storageRef.child("users/${getYourUserId()}")
