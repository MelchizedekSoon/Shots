...
class FakeReceivedShotRepositoryImpl @Inject
constructor(
    ...
    override suspend fun saveReceivedShot(
        ...
        if (success) {
            ...
            try {
                ...
                Log.d("FakeReceivedShotRepositoryImpl", "receivedShot was stored!")
                try {
                    receivedShotLocalDataSource.upsert(receivedShot)
                } catch (e: Exception) {
                    Log.d("FakeReceivedShotRepositoryImpl", "receivedShot failed to be stored!")
                }
            }
        } else {
            Log.d("FakeReceivedShotRepositoryImpl", "receivedShot failed to be stored!")
    ...
    override suspend fun fetchUpdatedReceivedShots(): Flow<List<String>> {
        ...
        Log.d("FakeReceivedShotRepositoryImpl", "inside fetchUpdatedReceivedShots where receivedShot = $receivedShot")
        Log.d("FakeReceivedShotRepositoryImpl", "inside fetchUpdatedReceivedShots where receivedShots = ${receivedShot.receivedShots}")
        receivedShotLocalDataSource.upsert(receivedShot)
