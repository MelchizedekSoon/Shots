...
class ReceivedShotRemoteDataSourceImpl @Inject
constructor(
    ...
    override suspend fun writeReceivedShotToFirebase(
        ...
        return try {
            ...
            Log.d("ReceivedShotRemoteDataSourceImpl", "downloadUrl = $downloadUrl")
            Log.d("ReceivedShotRemoteDataSourceImpl", "receivedShot added successfully")
            true
        } catch (e: Exception) {
            Log.d("ReceivedShotRemoteDataSourceImpl", "Error adding receivedShot: ${e.message}")
        ...
        receivedShotItems.forEach { (fieldName, uri) ->
            uri.let { validUri ->
                ...
                try {
                    ...
                    Log.d("ReceivedShotRemoteDataSourceImpl", "File successfully added to storage")
                    val finishedDownloadUrl = mediaRef.downloadUrl.await()
                    downloadUrl[fieldName] = finishedDownloadUrl
                } catch (e: Exception) {
                    Log.d(
                        "ReceivedShotRemoteDataSourceImpl",
...
//                        val receivedShotValue = value
    ...
    /** this has its own data sources and impl classes but this particular method is still attached
     * to receivedShot. I may change it at some point
     */
...
//                Log.d("ReceivedShotRemoteDataSourceImpl", "item.name = ${item.name}")
//                when {
...
//    }
    ...
    override suspend fun removeReceivedShotFromFirebase(userId: String?): Boolean {
        ...
        if (userId == null) {
            Log.d("ReceivedShotRemoteDataSourceImpl", "Invalid userId: null")
            return false // Error
        }
        ...
        return try {
            ...
            val mediaList = mediaRef.listAll().await()
            Log.d("ReceivedShotRemoteDataSourceImpl", "Inside " +
...
//        }
//
//        Log.d("ReceivedShotRemoteDataSourceImpl", "userId: $userId")
//
//        return try {
//            val receivedShotRef = firebaseFirestore
//                .collection("users")
